...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.СборкаЗапасов.ПараметрыУказанияСерий(Объект));
...
Процедура ПриОткрытии(Отказ)
	...
	//+БИТ от 08.04.2026 Добавление реквизита "Рекламная продукция" в ТЧ "Продукция"
	Если Объект.Ссылка.Пустая() Тогда
		Для каждого СтрокаТЧ Из Объект.Продукция Цикл
		    ЗаполнитьРеквизитРекламнаяПродукция(,СтрокаТЧ);
		КонецЦикла;	
	КонецЕсли;
	//-БИТ от 08.04.2026 Добавление реквизита "Рекламная продукция" в ТЧ "Продукция"
...
Процедура ДокументОснованиеПриИзменении(Элемент)
	...
	Если Объект.ЗаказНаПроизводство <> ЗаказНаПроизводство Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ДокументОснованиеПриИзмененииЗавершить", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Документ будет полностью перезаполнен по заказу на производство, продолжить?'");
...
Процедура ПродукцияНоменклатураПриИзменении(Элемент)
	...
	//+БИТ от 08.04.2026 Добавление реквизита "Рекламная продукция" в ТЧ "Продукция"
	ЗаполнитьРеквизитРекламнаяПродукция();
	//-БИТ от 08.04.2026 Добавление реквизита "Рекламная продукция" в ТЧ "Продукция"
...
//+БИТ от 08.04.2026 Добавление реквизита "Рекламная продукция" в ТЧ "Продукция"
&НаКлиенте
Процедура ЗаполнитьРеквизитРекламнаяПродукция(ИмяТабличнойЧасти = "Продукция", СтрокаТабличнойЧасти = Неопределено)
	...
	ЗначениеДопРеквизитаРекламнаяПродукция = Строка(ПолучитьЗначениеДопРеквизита(Номенклатура));
	Если ЗначениеДопРеквизитаРекламнаяПродукция <> "" Тогда
		СтрокаТабличнойЧасти.РекламнаяПродукция = ЗначениеДопРеквизитаРекламнаяПродукция;
	Иначе
		СтрокаТабличнойЧасти.РекламнаяПродукция = "";
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьЗначениеДопРеквизита(НоменклатураСсылка)
	 
	ОбъектНоменклатуры = НоменклатураСсылка.ПолучитьОбъект();
	
	ЭлементПВХ = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "РекламнаяПродукция_9938cdda368c4e388d7310bfba4baa8d");
	...
	Если СвойствоЗначение <> Неопределено Тогда
	    ЗначениеДопРеквизитаРекламнаяПродукция = СвойствоЗначение.Значение; 
	Иначе
		ЗначениеДопРеквизитаРекламнаяПродукция = Неопределено;
	КонецЕсли;
	Если ЗначениеДопРеквизитаРекламнаяПродукция <> Неопределено Тогда
		Возврат ЗначениеДопРеквизитаРекламнаяПродукция;
	Иначе
		Возврат "";
	КонецЕсли;
...
Процедура ОпределитьВидимостьНастроекУчетаВНалогообложении()
	...
	УчитыватьВНУ = ПерсональныеНастройкиРУНФ.Прочитать("УСНПроизводствоПоУмолчанию") И ВидимостьРасходыУСН;
...
&НаКлиенте
Процедура ДокументОснованиеПриИзмененииЗавершить(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		ЗаполнитьПоОснованиюЗавершение(Результат, ДополнительныеПараметры);
...
Процедура ДокументОснованиеПриИзмененииСервер()
	...
	ЗаказНаПроизводство = Объект.ЗаказНаПроизводство;
	ЗаполнитьУчетПотребности();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьУчетПотребности()
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("УчетПотребностиПоСкладам");
	
	Если ЗначениеЗаполнено(Объект.ЗаказНаПроизводство) Тогда
		ДокОснование = Объект.ЗаказНаПроизводство;
	Иначе
		ДокОснование = Неопределено;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДокОснование) Тогда
		ДанныеЗаполнения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокОснование, Новый ФиксированнаяСтруктура(ДанныеЗаполнения));
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Если ЭтоРазборка Тогда
		...
		Элементы.СтруктурнаяЕдиницаЗапасовРазборка.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ЗаказНаПроизводство) И Форма.УчетПотребностиПоСкладам;
		Элементы.ЗапасыСтруктурнаяЕдиница.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ЗаказНаПроизводство) И Форма.УчетПотребностиПоСкладам;
		Элементы.СтруктурнаяЕдиницаПродукцииСборка.ТолькоПросмотр = Ложь;
		Элементы.ПродукцияСтруктурнаяЕдиница.ТолькоПросмотр = Ложь;
	...
	Иначе
		...
		Элементы.СтруктурнаяЕдиницаЗапасовРазборка.ТолькоПросмотр = Ложь;
		Элементы.ЗапасыСтруктурнаяЕдиница.ТолькоПросмотр = Ложь;
		Элементы.СтруктурнаяЕдиницаПродукцииСборка.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ЗаказНаПроизводство) И Форма.УчетПотребностиПоСкладам;
		Элементы.ПродукцияСтруктурнаяЕдиница.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ЗаказНаПроизводство) И Форма.УчетПотребностиПоСкладам;
