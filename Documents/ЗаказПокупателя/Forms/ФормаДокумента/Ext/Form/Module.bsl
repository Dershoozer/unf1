...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПодборПроизведен" 
		...
		Если МаркерПодбора = "Запасы" Тогда
			...
			//+БИТ от 08.04.2026 расчет короба
			Для каждого СтрокаТЧ Из Объект.Запасы Цикл
			    РассчитатьКоробВСтрокеТабличнойЧасти(, СтрокаТЧ);
			КонецЦикла;
			//-БИТ от 08.04.2026 расчет короба
			СтруктураДанные = Новый Структура;
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент)
	...
	//+БИТ от 08.04.2026 расчет короба
	РассчитатьКоробВСтрокеТабличнойЧасти();
	//-БИТ от 08.04.2026 расчет короба
...
//+БИТ от 08.04.2026 расчет короба
&НаКлиенте
Процедура РассчитатьКоробВСтрокеТабличнойЧасти(ИмяТабличнойЧасти = "Запасы", СтрокаТабличнойЧасти = Неопределено)
	...
	ЗначениеДопРеквизитаКолвоВКоробке = Число(ПолучитьЗначениеДопРеквизита(Номенклатура));
	Если ЗначениеДопРеквизитаКолвоВКоробке > 0 Тогда
		СтрокаТабличнойЧасти.Короб = Количество/ЗначениеДопРеквизитаКолвоВКоробке;
	Иначе
		СтрокаТабличнойЧасти.Короб = 0;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовПоНастройкеПользователя(Форма)
	...
	//+БИТ от 08.04.2026 расчет короба
    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОбщееКоличествоКоробов",
		"Видимость", Форма.ПоказыватьКолонкуВесВЗаказеПокупателя);
	//-БИТ от 08.04.2026 расчет короба
...
Процедура ЗагрузитьИзФайлаЗавершение(АдресЗагруженныхДанных) Экспорт
	...
	Для каждого СтрокаТаблицы Из Объект.Запасы Цикл 
		...
		//+БИТ от 08.04.2026 расчет короба
		РассчитатьКоробВСтрокеТабличнойЧасти(, СтрокаТаблицы);
		//-БИТ от 08.04.2026 расчет короба
	КонецЦикла;
...
Процедура РассчитатьОбъемИВесЗаказа()
	...
	Для Каждого ТекСтрока Из СтрокиТекущегоВарианта Цикл
		...
		ОбъемИтог = ОбъемИтог + ТекСтрока.Объем;
		//+БИТ от 08.04.2026 расчет короба
		ОбщееКоличествоКоробов = ОбщееКоличествоКоробов + ТекСтрока.Короб;
		//-БИТ от 08.04.2026 расчет короба
	КонецЦикла;
	...
	КонецЕсли;
	//+БИТ от 08.04.2026 расчет короба
	Если ЗначениеЗаполнено(ОбщееКоличествоКоробов) Тогда
		Объект.ОбщееКоличествоКоробов = ОбщееКоличествоКоробов;
	КонецЕсли;
	//-БИТ от 08.04.2026 расчет короба
	ЗаполнитьЗаголовокГруппыГабариты(ЭтотОбъект);
