
&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	 //////////////////////////////////
	// Получаем параметры
	Если Параметры.Свойство("МассивКодовМаркировкиНеотсканированных") Тогда
		МассивКодовМаркировкиНеотсканированных = Новый ХранилищеЗначения(Параметры.МассивКодовМаркировкиНеотсканированных);
		//МассивКодовМаркировкиНеотсканированных = Параметры.МассивКодовМаркировкиНеотсканированных;
	КонецЕсли;
	Если Параметры.Свойство("СтрокаНумерацииУпакованных") Тогда
		Если Параметры.СтрокаНумерацииУпакованных <>"" ИЛИ Параметры.СтрокаНумерацииУпакованных <> Неопределено Тогда
			СтрокаНумерацииУпакованных = Параметры.СтрокаНумерацииУпакованных;
		КонецЕсли;
	КонецЕсли;
	Если Параметры.Свойство("Тип") Тогда
		Тип = Параметры.Тип;
		Элементы.КартинкаСканирования.Видимость = Истина;
		Если Тип = "Набор" Тогда 
			//ТекстПодсказки = "Наклейте КМ на упаковку и отсканируйте " + МассивКодовМаркировкиНеотсканированных.Получить()[0];
			Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКИНУупаковки;
			СтрокаНумерацииУпакованных = "Набор собран";
		КонецЕсли;
		Если Тип = "ЧастьНабора" Тогда
			//ТекстПодсказки = "Отсканируйте ШТРИХКОД части набора";
			Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияШКЧастиНабора;
		КонецЕсли;
		Если Тип = "Дойпак" Тогда
			//ТекстПодсказки = "Отсканируйте ШТРИХКОД дойпака";
			Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияШКДойпака;
		КонецЕсли;
	КонецЕсли;
	Если Параметры.Свойство("НоменклатураСборки") Тогда
		НоменклатураСборки = Строка(Параметры.НоменклатураСборки);
	КонецЕсли; 
	Если Параметры.Свойство("ШтрихкодНоменклатураСборки") Тогда
		Если Тип = "ЧастьНабора" Тогда
			МассивКМ = МассивКодовМаркировкиНеотсканированных.Получить();
			Для Каждого СтрокаМасиваКМ Из МассивКМ Цикл
				ШтрихкодЧастиНабора = ПолучитьОсновнойШКПоКодуМаркировки(СтрокаМасиваКМ);
				НоменклатураСборки = НоменклатураСборки + " (" + Строка(ШтрихкодЧастиНабора) + ") ";
			КонецЦикла;
		Иначе
			НоменклатураСборки = НоменклатураСборки + " (" + Строка(Параметры.ШтрихкодНоменклатураСборки) + ")";
		КонецЕсли;
	КонецЕсли;
	
	
	ТребуемыйКодМаркировки = "";
	ШтрихкодУжеВведен = Ложь;
	КодМаркировкиПоДойпакуУжеВведен = Ложь;
	 /////////////////////////////
	//ТекстПодсказки = "Отсканируйте ШТРИХКОД дойпака";
	//// Создаем форматированную строку
	//ФормСтрока = Новый ФорматированнаяСтрока();

	//// Добавляем первую часть с подчеркиванием
	//Часть1 = ФормСтрока.Добавить("КМ УПАКОВКИ");
	//Часть1.Шрифт = Новый Шрифт(,,,Истина,,,Истина); // Жирный и Подчеркнутый

	//// Добавляем вторую часть без подчеркивания
	//Часть2 = ФормСтрока.Добавить(" выбрать");
	//Часть2.Шрифт = Новый Шрифт(,,,Ложь,,,Ложь); // Обычный

	// Выводим на форму
	//ТекстПодсказки = "<b><u>КМ УПАКОВКИ</u></b> выбрать";
	
	
	
	// Получаем параметры
	Если Параметры.Свойство("ТекстПодсказки") Тогда
		ТекстПодсказки = Параметры.ТекстПодсказки;
	КонецЕсли;
	
	Если Параметры.Свойство("ОжидаемыйКод") Тогда
		ОжидаемыйКод = Параметры.ОжидаемыйКод;
	КонецЕсли;
	
	Если Параметры.Свойство("ТипВвода") Тогда
		ТипВвода = Параметры.ТипВвода;
	КонецЕсли;
	
КонецПроцедуры


&НаКлиенте
Процедура ПриОткрытии(Отказ)
    // Устанавливаем обработчик ожидания, который сработает через 1 секунду
    WSHShell = Новый COMОбъект("WScript.Shell");
    WSHShell.SendKeys("% ");
    WSHShell.SendKeys("{DOWN}{DOWN}{DOWN}{DOWN}{ENTER}");
	Если Тип = "Набор" Тогда
		ПечатьКМ(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0],"КИН");
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ПолучитьОсновнойШКПоКодуМаркировки(КодМаркировки)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	УпаковкаПродукцииКодыМаркировки.Номенклатура.Штрихкод КАК НоменклатураШтрихкод
	               |ИЗ
	               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
	               |ГДЕ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки = &КодМаркировки";
	Запрос.УстановитьПараметр("КодМаркировки",КодМаркировки);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.НоменклатураШтрихкод;
	КонецЕсли;
	Возврат Неопределено;
КонецФункции

&НаСервере
Функция ПолучитьМассивИзХранилища(Хранилище)
    Если Хранилище = Неопределено Тогда
        Возврат Новый Массив;
    КонецЕсли;
    Возврат Хранилище.Получить();
КонецФункции

&НаКлиенте
Процедура КнопкаОКНажатие(Команда)
	//////////Обработка набора
	Если Тип = "Набор" Тогда
		Если ВведенныйКод = ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0] Тогда
			ОчиститьСообщения();
			ТекстПодсказки = "КМ верный для набора " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
			ОзвучитьТекст("Упаковка собрана");
			Закрыть(ВведенныйКод);
		Иначе
			ОчиститьСообщения();
			ТекстПодсказки = "Ошибка: Введен неверный КМ для набора " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
			HTMLЗвук = "";
			ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
			ВведенныйКод = "";
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	//////////Обработка части набора
	Если Тип = "ЧастьНабора" Тогда 
		Если Не ШтрихкодУжеВведен Тогда
			МассивШтрихкодов = ПолучитьМассивШтрихкодовИGTINПоКодамМаркировки(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных));
			ИндексМассива = МассивШтрихкодов.Найти(ВведенныйКод);
			Если ИндексМассива <> Неопределено Тогда 
				ОчиститьСообщения();
				ТекстПодсказки = "ШТРИХКОД верный для части набора " + МассивШтрихкодов[ИндексМассива];
				ТребуемыйКодМаркировки = ПолучитьКодМаркировкиПоШтрихкодуИЛИGTIN(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных), ВведенныйКод);
				ШтрихкодУжеВведен = Истина;
				//ТекстПодсказки = "Наклейте КМ на часть набора и отсканируйте"; 
				Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКМЧастиНабора;
				ПечатьКМ(ТребуемыйКодМаркировки,"КМ");
				ВведенныйКод = "";
				Возврат;
			Иначе 
				ОчиститьСообщения();
				ТекстПодсказки = "Ошибка: Введен неверный ШТРИХКОД для части набора";
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				ВведенныйКод = "";
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		
		Если ВведенныйКод = ТребуемыйКодМаркировки Тогда
			ОчиститьСообщения();
			ТекстПодсказки = "КМ верный для части набора " + ТребуемыйКодМаркировки;
			Закрыть(ВведенныйКод);
		Иначе
			ОчиститьСообщения();
			ТекстПодсказки = "Ошибка: Введен неверный КМ для части набора " + Строка(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0]);
			HTMLЗвук = "";
			ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
			ВведенныйКод = "";
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	//////////Обработка дойпака
	Если Тип = "Дойпак" Тогда
		Если Не ШтрихкодУжеВведен Тогда///Вводим сначала штрихкод для дойпака
			МассивШтрихкодов = ПолучитьМассивШтрихкодовИGTINПоКодамМаркировки(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных));
			ИндексМассива = МассивШтрихкодов.Найти(ВведенныйКод);
			Если ИндексМассива <> Неопределено Тогда
				ОчиститьСообщения();
				ТекстПодсказки = "ШТРИХКОД верный для дойпака " + МассивШтрихкодов[ИндексМассива];
				ТребуемыйКодМаркировки = ПолучитьКодМаркировкиПоШтрихкодуИЛИGTIN(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных), ВведенныйКод);
				ШтрихкодУжеВведен = Истина;
				//ТекстПодсказки = "Наклейте КМ на дойпак и отсканируйте";
				Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКМДойпака;
				ПечатьКМ(ТребуемыйКодМаркировки,"КМ");
				ВведенныйКод = "";
				Возврат;
			Иначе
				ОчиститьСообщения();
				ТекстПодсказки = "Ошибка: Введен неверный ШТРИХКОД для дойпака";
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				ВведенныйКод = "";
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		Если Не КодМаркировкиПоДойпакуУжеВведен Тогда///Вводим после КМ для дойпака
			Если ВведенныйКод = ТребуемыйКодМаркировки Тогда
				ОчиститьСообщения();
				ТекстПодсказки = "КМ верный для дойпака " + ТребуемыйКодМаркировки;
				КодМаркировкиПоДойпакуУжеВведен = Истина;
				//ТекстПодсказки = "Наклейте КМ на упаковку дойпака и отсканируйте(дубль)";
				Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКМдубляДляУпаковкиДойпака;
				ПечатьКМ(ТребуемыйКодМаркировки,"КМ");
				ВведенныйКод = "";
				Возврат;
			Иначе
				ОчиститьСообщения();
				ТекстПодсказки = "Ошибка: Введен неверный КМ для дойпака " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				ВведенныйКод = "";
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		Если ВведенныйКод = ТребуемыйКодМаркировки Тогда///Вводим после КМ для упаковки дойпака и теперь закрываем
			ОчиститьСообщения();
			ТекстПодсказки = "КМ верный для упаковки дойпака " + ТребуемыйКодМаркировки;
			ОзвучитьТекст("Упаковка собрана");
			Закрыть(ВведенныйКод);
		Иначе
			ОчиститьСообщения();
			ТекстПодсказки = "Ошибка: Введен неверный КМ для упаковки дойпака " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
			HTMLЗвук = "";
			ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
			ВведенныйКод = "";
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьМассивШтрихкодовИGTINПоКодамМаркировки(МассивКМ)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	УпаковкаПродукцииКодыМаркировки.GTIN КАК GTIN
	               |ИЗ
	               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
	               |ГДЕ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки В(&МассивКМ)";
	Запрос.УстановитьПараметр("МассивКМ",МассивКМ);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	МассивШК = Новый Массив;
	Пока Выборка.Следующий() Цикл
		МассивШК.Добавить(Выборка.GTIN);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ШтрихкодыНоменклатуры.Штрихкод КАК Штрихкод
	               |ИЗ
	               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	               |		ПО УпаковкаПродукцииКодыМаркировки.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	               |ГДЕ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки В(&МассивКМ)";
	Запрос.УстановитьПараметр("МассивКМ",МассивКМ);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		МассивШК.Добавить(Выборка.Штрихкод);
	КонецЦикла;
	
	Возврат МассивШК;
КонецФункции

&НаСервере
Функция ПолучитьКодМаркировкиПоШтрихкодуИЛИGTIN(МассивКМ, Штрихкод)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	УпаковкаПродукцииКодыМаркировки.Номенклатура КАК Номенклатура,
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки КАК КодМаркировки
	               |ИЗ
	               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
	               |ГДЕ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки В(&МассивКМ)
	               |	И УпаковкаПродукцииКодыМаркировки.GTIN = &GTIN";
	Запрос.УстановитьПараметр("МассивКМ",МассивКМ);
	Запрос.УстановитьПараметр("GTIN",Штрихкод);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Если Выборка.Следующий() Тогда
		Возврат Выборка.КодМаркировки;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки КАК КодМаркировки
	               |ИЗ
	               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	               |		ПО УпаковкаПродукцииКодыМаркировки.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	               |ГДЕ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки В(&МассивКМ)
	               |	И ШтрихкодыНоменклатуры.Штрихкод = &Штрихкод";
	Запрос.УстановитьПараметр("МассивКМ",МассивКМ);
	Запрос.УстановитьПараметр("Штрихкод",Штрихкод);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Если Выборка.Следующий() Тогда
		Возврат Выборка.КодМаркировки;
	КонецЕсли;

	
	Возврат Неопределено;
КонецФункции

&НаКлиенте
Процедура КнопкаОтменаНажатие(Команда)
	Закрыть(Неопределено);
КонецПроцедуры


&НаКлиенте
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	Если Не ВводДоступен() Тогда
		Сообщить("Ввод не доступен");
		Возврат;
	КонецЕсли;
	
	// Обработка сканера
	//Источник пример:"InputDevice#727045279231", событие пример:"Штрихкод" - типы "Строка"
	Если Событие = "Штрихкод" Тогда
		Данные = Строка(Данные);
		
		Если ШтрихкодУжеВведен = Ложь И Тип <> "Набор" Тогда//если шквведен и частьнабора или дойпак
			Если СтрДлина(Данные) > 15 Тогда//у шк длина 14
				ТекстПодсказки = "Вы сканируете КМ, а необходимо ШТРИХКОД!";
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				Возврат;
			КонецЕсли;
			//Пример: 01 04634089754019 21 5c3ToxC1"Q0"L   91EE1292sb8lG47KhSmv8WcyFaEhls4sgtrGrfshkcHS2dMDTb4=
			//0104607035653292215Ql z>c 93bgux
			//Обрабатываем данные на клиенте, на сервер передать нельзя - нераспознанный символ для 1С(преобразование XML)
			//Данные = Сред(Данные, 0, 31);
			//Данные = "("+Сред(Данные,0,2)+")"+Сред(Данные,3,14)+"("+Сред(Данные,17,2)+")"+Сред(Данные,19,13);
			//Данные = ПолучитьКодМаркировкиПоДаннымСканера(Данные);
		Иначе
			Если СтрДлина(Данные) < 15 Тогда//у шк длина 14
				ТекстПодсказки = "Вы сканируете ШТРИХКОД, а необходимо КМ!";
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				Возврат;
			КонецЕсли; 
			Данные = "("+Сред(Данные,0,2)+")"+Сред(Данные,3,14)+"("+Сред(Данные,17,2)+")"+Сред(Данные,19,13);
		КонецЕсли;
		
			//////////Обработка набора
		Если Тип = "Набор" Тогда
			Если Данные = ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0] Тогда
				ОчиститьСообщения();
				ТекстПодсказки = "КМ верный для набора " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
				ОзвучитьТекст("Упаковка собрана");
				Закрыть(Данные);
			Иначе
				ОчиститьСообщения();
				ТекстПодсказки = "Ошибка: Введен неверный КМ для набора " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				ВведенныйКод = "";
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		//////////Обработка части набора
		Если Тип = "ЧастьНабора" Тогда 
			Если Не ШтрихкодУжеВведен Тогда
				МассивШтрихкодов = ПолучитьМассивШтрихкодовИGTINПоКодамМаркировки(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных));
				ИндексМассива = МассивШтрихкодов.Найти(Данные);
				Если ИндексМассива <> Неопределено Тогда 
					ОчиститьСообщения();
					ТекстПодсказки = "ШТРИХКОД верный для части набора " + МассивШтрихкодов[ИндексМассива];
					ТребуемыйКодМаркировки = ПолучитьКодМаркировкиПоШтрихкодуИЛИGTIN(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных), Данные);
					ШтрихкодУжеВведен = Истина;
					//ТекстПодсказки = "Наклейте КМ на часть набора и отсканируйте";
					Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКМЧастиНабора;
					ПечатьКМ(ТребуемыйКодМаркировки,"КМ");
					ВведенныйКод = "";
					Возврат;
				Иначе 
					ОчиститьСообщения();
					ТекстПодсказки = "Ошибка: Введен неверный ШТРИХКОД для части набора";
					HTMLЗвук = "";
					ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
					ВведенныйКод = "";
					Возврат;
				КонецЕсли;
			КонецЕсли;
			
			
			Если Данные = ТребуемыйКодМаркировки Тогда
				ОчиститьСообщения();
				ТекстПодсказки = "КМ верный для части набора " + ТребуемыйКодМаркировки;
				Закрыть(Данные);
			Иначе
				ОчиститьСообщения();
				ТекстПодсказки = "Ошибка: Введен неверный КМ для части набора " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				ВведенныйКод = "";
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		//////////Обработка дойпака
		Если Тип = "Дойпак" Тогда
			Если Не ШтрихкодУжеВведен Тогда
				МассивШтрихкодов = ПолучитьМассивШтрихкодовИGTINПоКодамМаркировки(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных));
				ИндексМассива = МассивШтрихкодов.Найти(Данные);
				Если ИндексМассива <> Неопределено Тогда
					ОчиститьСообщения();
					ТекстПодсказки = "ШТРИХКОД верный для дойпака " + МассивШтрихкодов[ИндексМассива];
					ТребуемыйКодМаркировки = ПолучитьКодМаркировкиПоШтрихкодуИЛИGTIN(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных), Данные);
					ШтрихкодУжеВведен = Истина;
					//ТекстПодсказки = "Наклейте КМ на дойпак и отсканируйте";
					Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКМДойпака;
					ПечатьКМ(ТребуемыйКодМаркировки,"КМ");
					ВведенныйКод = "";
					Возврат;
				Иначе
					ОчиститьСообщения();
					ТекстПодсказки = "Ошибка: Введен неверный ШТРИХКОД для дойпака";
					HTMLЗвук = "";
					ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
					ВведенныйКод = "";
					Возврат;
				КонецЕсли;
			КонецЕсли; 
			
			Если Не КодМаркировкиПоДойпакуУжеВведен Тогда///Вводим после КМ для дойпака
				Если Данные = ТребуемыйКодМаркировки Тогда
					ОчиститьСообщения();
					ТекстПодсказки = "КМ верный для дойпака " + ТребуемыйКодМаркировки; 
					КодМаркировкиПоДойпакуУжеВведен = Истина;
					//ТекстПодсказки = "Наклейте КМ на упаковку дойпака и отсканируйте";
					Элементы.КартинкаСканирования.Картинка = БиблиотекаКартинок.БИТ_КартинкаСканированияКМдубляДляУпаковкиДойпака;
					ПечатьКМ(ТребуемыйКодМаркировки,"КМ");
					ВведенныйКод = "";
					Возврат;
				Иначе
					ОчиститьСообщения();
					ТекстПодсказки = "Ошибка: Введен неверный КМ для дойпака " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
					HTMLЗвук = "";
					ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
					ВведенныйКод = "";
					Возврат;
				КонецЕсли;
			КонецЕсли;
			
			
			Если Данные = ТребуемыйКодМаркировки Тогда
				ОчиститьСообщения();
				ТекстПодсказки = "КМ верный для дойпака " + ТребуемыйКодМаркировки;
				ОзвучитьТекст("Упаковка собрана");
				Закрыть(Данные);
			Иначе
				ОчиститьСообщения();
				ТекстПодсказки = "Ошибка: Введен неверный КМ для дойпака " + ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0];
				HTMLЗвук = "";
				ПодключитьОбработчикОжидания("ПроигратьЗвукОшибкиСканирования" ,0.1, Истина); // 0.1 секунды задержки
				ВведенныйКод = "";
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьШтрихкодПоДаннымСканера(Данные)
    // Ищем маркер GTIN (01)
	//ПозицияНачала = Найти(Данные, "(01)");
	//
	//Если ПозицияНачала > 0 Тогда
	//	// Извлекаем 14 цифр после (01)
	//	Штрихкод = Сред(Данные, ПозицияНачала + 4, 14);
	//	Возврат Штрихкод;
	//КонецЕсли;
	//
	//// Если не нашли, пробуем другие форматы
	//ПозицияНачала = Найти(Данные, "01");
	//Если ПозицияНачала > 0 Тогда
	//	Штрихкод = Сред(Данные, ПозицияНачала + 2, 14);
	//	Возврат Штрихкод;
	//КонецЕсли;
	//
	//// Если не нашли, данные без скобок - срезаем первые 2 цифры и последние
	//Если ПозицияНачала > 0 Тогда
	//	Штрихкод = Сред(Данные, 2, 14);
	//	Возврат Штрихкод;
	//КонецЕсли;
	Штрихкод = Данные;
	Возврат Штрихкод;
КонецФункции

&НаСервере
Функция ПолучитьШКИзКодаМаркировки(Данные)
    // Ищем маркер GTIN (01)
	ПозицияНачала = Найти(Данные, "(01)");
	
	Если ПозицияНачала > 0 Тогда
		// Извлекаем 14 цифр после (01)
		КодМаркировки = Сред(Данные, ПозицияНачала + 4, 14);
		Возврат КодМаркировки;
	КонецЕсли;
	
	// Если не нашли, пробуем другие форматы
	ПозицияНачала = Найти(Данные, "01");
	Если ПозицияНачала > 0 Тогда
		КодМаркировки = Сред(Данные, ПозицияНачала + 2, 14);
		Возврат КодМаркировки;
	КонецЕсли;
	
	Возврат "";
КонецФункции

&НаКлиенте
Процедура ОзвучитьТекст(ТекстДляОзвучки)
	
	Попытка
		// 1. Создаем COM-объект голосового движка Windows
		Голос = Новый COMОбъект("SAPI.SpVoice");
		
		// Пример для голоса "Pavel" из Windows 10/11
		Голос.Voice = Голос.GetVoices().Item(0); // Первый доступный голос
		// Или указать конкретный из реестра:
		// Голос.Voice.Category.Default = "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech_OneCore\Voices\Tokens\MSTTS_V110_ruRU_PavelM";
		
		//// берем последний
		//Для Индекс = 0 По Голос.GetVoices().Count - 1 Цикл
		//	Голос.Voice = Голос.GetVoices().Item(Индекс);
		//КонецЦикла;
		
		// 2. Устанавливаем громкость (0 - 100)
		Голос.Volume = 100;
		
		// 3. Выбираем скорость речи (-10 - 10). 0 - средняя
		Голос.Rate = 0;
		
		// 4. Произносим текст
		Голос.Speak(ТекстДляОзвучки);
		
	Исключение
		Сообщить("Не удалось воспроизвести речь: " + ОписаниеОшибки());
	КонецПопытки;
	
КонецПроцедуры

///////////////////////////////////ПЕЧАТЬ/////////////////////////////////////////////////////////////////////
&НаКлиенте
Процедура ПечатьКМ(ВведенныйШтрихкод, ШаблонЭтикеткиКИНИлиКМ)
	
	Если Не ЗначениеЗаполнено(ВведенныйШтрихкод) Тогда
		Сообщить("Введите штрихкод");
		Возврат;
	КонецЕсли;
	
	// Поиск упаковки по штрихкоду на сервере
	НайденнаяУпаковка = ПоискУпаковкиПоШтрихкодуНаСервере(ВведенныйШтрихкод);
	
	Если НайденнаяУпаковка = "" Тогда
		Сообщить("Упаковка со штрихкодом """ + ВведенныйШтрихкод + """ не найдена");//убрать в сообщение
		Возврат;
	КонецЕсли;
	
	// Получаем шаблон этикетки из справочника ХранилищеШаблонов
	ШаблонЭтикетки = ПолучитьШаблонЭтикеткиНаСервере(ШаблонЭтикеткиКИНИлиКМ);
	
	Если ШаблонЭтикетки = "" Тогда
		Сообщить("Не найден шаблон этикетки");
		Возврат;
	КонецЕсли;
	
	// Формируем описание команды для печати
	ОписаниеКоманды = Новый Структура;
	ОписаниеКоманды.Вставить("Идентификатор", "ЭтикеткаКодМаркировкиИСМП");
	ОписаниеКоманды.Вставить("ОбъектыПечати", Новый Массив);
	ОписаниеКоманды.ОбъектыПечати.Добавить(НайденнаяУпаковка);
	ОписаниеКоманды.Вставить("Форма", ЭтаФорма); 
	ОписаниеКоманды.Вставить("ШаблонЭтикетки", ШаблонЭтикетки);
	
	// Получаем данные для печати
	ОбъектыПечати = ПечатьЭтикетокИСМПВызовСервера.ДанныеДляПечатиШтрихкодовУпаковокИСМП(
		ОписаниеКоманды.Идентификатор,
		ОписаниеКоманды.ОбъектыПечати,
		Новый Массив()); // МассивШаблоновКодовМаркировок сразу создаем
	
	Если ОбъектыПечати.Количество() = 0 Тогда
		Сообщить(НСтр("ru='Нет данных для печати этикеток.'"));
		Возврат;
	КонецЕсли;
	
	// Формируем параметры открытия формы
	ПараметрыОткрытия = Новый Структура(
		"АдресВХранилище, Шаблоны", 
		ПоместитьВоВременноеХранилище(Новый Структура("РежимПечати, ОбъектыПечати", "Выборочно", ОбъектыПечати)),
		Новый Массив());
		
		
	ПараметрыОткрытия.Вставить("СразуНаПринтер", Истина);
	ПараметрыОткрытия.Вставить("ШаблонЭтикетки", ШаблонЭтикетки);
	// Открываем форму
	//ОткрытьФорму("Справочник.ШаблоныЭтикетокИЦенников.Форма.ФормаШтрихкодыЭтикетокИСМП", ПараметрыОткрытия, ЭтаФорма);
	
	
	ДанныеП = выполнитьПечатьНаСервере(ПараметрыОткрытия);
	ВыполнитьПечатьДока(ДанныеП);
КонецПроцедуры

&НаСервере
Функция выполнитьПечатьНаСервере(ПараметрыОткрытия)
	СразуНаПринтер        = ПараметрыОткрытия.СразуНаПринтер;
	КоличествоЭкземпляров = 1;
	ШаблонЭтикетки        = ПараметрыОткрытия.ШаблонЭтикетки;
	
	Если ЗначениеЗаполнено(ПараметрыОткрытия.АдресВХранилище) Тогда
	АдресВХранилище = ПоместитьВоВременноеХранилище(
		ПолучитьИзВременногоХранилища(ПараметрыОткрытия.АдресВХранилище),
		УникальныйИдентификатор);
	КонецЕсли;
	
	ДанныеПечати = ПолучитьИзВременногоХранилища(АдресВХранилище);
	Для Каждого СтрокаПечати Из ДанныеПечати.ОбъектыПечати Цикл
		СтрокаПечати.ШаблонЭтикетки = ШаблонЭтикетки;
	КонецЦикла;
	
	Возврат ДанныеПечати;
	
КонецФункции

&НаКлиенте
Процедура ВыполнитьПечатьДока(ДанныеП)
	
	ПараметрКоманды = Новый Массив;
	ПараметрКоманды.Добавить(ПредопределенноеЗначение("Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка"));
	
	ПараметрыПечати = Новый Структура;
	ПараметрыПечати.Вставить("КоличествоЭкземпеляров", 1);
	ПараметрыПечати.Вставить("СтруктураДанных", ДанныеП);
	ПараметрыПечати.Вставить("АдресВХранилище", "");
	ПараметрыПечати.Вставить("КаждаяЭтикеткаНаНовомЛисте", Истина);
	ПараметрыПечати.Вставить("СразуНаПринтер", Истина);
	ПараметрыПечати.Вставить("РежимПечати", "ЭтикеткаКодМаркировкиИСМП");
	
	Если КонстантаПечататьСразуНаПринтер() Тогда
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечатиНаПринтер("Обработка.ПечатьЭтикетокИЦенников", "ЭтикеткаКодМаркировкиИСМП", ПараметрКоманды, ПараметрыПечати);
	Иначе
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.ПечатьЭтикетокИЦенников","ЭтикеткаКодМаркировкиИСМП",ПараметрКоманды,ВладелецФормы,ПараметрыПечати);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция КонстантаПечататьСразуНаПринтер()
	Возврат Константы.БИТ_ОтправкаПечатиКМКИНУШКсразуНаПринтер.Получить();
КонецФункции

&НаСервере
Функция ПоискУпаковкиПоШтрихкодуНаСервере(Знач ВведенныйШтрихкод)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ШтрихкодыУпаковокТоваров.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
		|ГДЕ
		|	ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода = &ЗначениеШтрихкода
		|	ИЛИ ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода = &ЗначениеШтрихкодаУпаковки";
	
	Запрос.УстановитьПараметр("ЗначениеШтрихкода", ВведенныйШтрихкод);
	Запрос.УстановитьПараметр("ЗначениеШтрихкодаУпаковки", ВведенныйШтрихкод);
	
	Результат = Запрос.Выполнить();
	
	Если Результат.Пустой() Тогда
		Возврат "";
	КонецЕсли;
	
	Выборка = Результат.Выбрать();
	Выборка.Следующий();
	
	Возврат Выборка.Ссылка;
	
КонецФункции  

&НаСервере
Функция ПолучитьШаблонЭтикеткиНаСервере(ШаблонЭтикеткиКИНИлиКМ)
	
	// Ищем шаблон по имени или типу
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ХранилищеШаблонов.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ХранилищеШаблонов КАК ХранилищеШаблонов
		|ГДЕ
		|	ХранилищеШаблонов.Наименование = &Наименование";
	
	Запрос.УстановитьПараметр("Наименование", ШаблонЭтикеткиКИНИлиКМ);
	
	Результат = Запрос.Выполнить();
	
	Если Не Результат.Пустой() Тогда
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
		Возврат Выборка.Ссылка;
	КонецЕсли;
	
	
	// Если не нашли, возвращаем пустую строку
	Возврат "";
	
КонецФункции


&НаКлиенте
Процедура ПовторнаяПечатьКМ(Команда)//код маркировки части набора или дойпака
	Если ТребуемыйКодМаркировки <> "" ИЛИ ТребуемыйКодМаркировки <> Неопределено Тогда
		ПечатьКМ(ТребуемыйКодМаркировки, "КМ");
	Иначе
		Сообщить("Еще не отсканирован Штрихкод дойпака!");
	КонецЕсли;
КонецПроцедуры


&НаКлиенте
Процедура ПовторнаяПечатьКИНУ(Команда)//код маркировки набора
	Если Тип <> "Дойпак" Тогда//теперь нужно понять это часть набора или сам набор
		Если Тип ="Набор" Тогда
			ПечатьКМ(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных)[0],"КИН");
		КонецЕсли;
		Если Тип = "ЧастьНабора" Тогда
			КодМаркировкиНабора = ПолучитьКодМаркировкиНабора(ПолучитьМассивИзХранилища(МассивКодовМаркировкиНеотсканированных));
			Если КодМаркировкиНабора <> Неопределено Тогда
				ПечатьКМ(КодМаркировкиНабора, "КИН");
			Иначе
				Сообщить("Не найден КИНУ!");
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ПолучитьКодМаркировкиНабора(МассивКМ)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	УпаковкаПродукцииКодыМаркировки.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
	               |	УпаковкаПродукцииКодыМаркировки.ДокументОснование КАК ДокументОснование,
	               |	УпаковкаПродукцииКодыМаркировки.НомерСтрокиВДокументеОсновании КАК НомерСтрокиВДокументеОсновании
	               |ИЗ
	               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
	               |ГДЕ
	               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки В(&МассивКМ)";
	Запрос.УстановитьПараметр("МассивКМ",МассивКМ);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Если Выборка.Следующий() Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               |	УпаковкаПродукцииКодыМаркировки.КодМаркировки КАК КодМаркировки
		               |ИЗ
		               |	РегистрСведений.УпаковкаПродукцииКодыМаркировки КАК УпаковкаПродукцииКодыМаркировки
		               |ГДЕ
		               |	УпаковкаПродукцииКодыМаркировки.ШтрихкодУпаковки = &ШКУпаковки
		               |	И УпаковкаПродукцииКодыМаркировки.ДокументОснование = &ДокументОснование
		               |	И УпаковкаПродукцииКодыМаркировки.НомерСтрокиВДокументеОсновании = &НомерСтрокиВДокументеОсновании
		               |	И УпаковкаПродукцииКодыМаркировки.Тип = ""Набор""";
		Запрос.УстановитьПараметр("ШКУпаковки",Выборка.ШтрихкодУпаковки);
		Запрос.УстановитьПараметр("ДокументОснование",Выборка.ДокументОснование);
		Запрос.УстановитьПараметр("НомерСтрокиВДокументеОсновании",Выборка.НомерСтрокиВДокументеОсновании);
		Результат = Запрос.Выполнить();
		Выборка = Результат.Выбрать();
		
		Если Выборка.Следующий() Тогда
			Возврат Выборка.КодМаркировки;
		КонецЕсли;
	КонецЕсли;
	
	Возврат Неопределено;
	
КонецФункции

&НаКлиенте
Процедура ПроигратьЗвукОшибкиСканирования()
	//очищаем старое значение
	HTMLЗвук = "";
	//base64 преобразование звука
	Звук = "data:audio/mpeg;base64,";
	//записываем HTML
	HTMLЗвук = "<html><body><audio autoplay><source src='" + Звук + "' type='audio/mpeg'></audio></body></html>";
	
	//ВАРИАНТ 2
	//Звук = Новый COMОбъект("SAPI.SpVoice");
	//Голоса = Звук.GetVoices();
	//Для Индекс = 0 По Голоса.Count - 1 Цикл
	//    Если СтрНайти(Голоса.Item(Индекс).GetDescription(), "Male") > 0 Тогда //находим голос, у Дубровского на сервере их всего 2 на ОС
	//        Звук.Voice = Голоса.Item(Индекс);
	//        Прервать;
	//    КонецЕсли;
	//КонецЦикла;

	//// 2. Ускоряем речь (от -10 до 10, где 0 — норма)
	//Звук.Rate = 3;  // Можно поставить 5-7 для сильного ускорения
	//Звук.Volume = 100;  // Максимальная громкость
	//Звук.Speak("ТТТТТУУУУУУМММММ!"); // Голосовое оповещение
 КонецПроцедуры

