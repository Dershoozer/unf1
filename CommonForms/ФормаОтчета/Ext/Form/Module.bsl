...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно, ПослеПредупреждения)
	...
	//+БИТ от 29.04.2026
	 // Получаем параметры из настроек отчета
    ПараметрыКомпоновщика = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных;
    
    // Находим параметры НачалоПериода и КонецПериода
    ПараметрНачало = ПараметрыКомпоновщика.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("НачалоПериода"));
    ПараметрКонец = ПараметрыКомпоновщика.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("КонецПериода"));
    
    // Если параметры существуют и не заполнены - устанавливаем значения по умолчанию
    Если ПараметрНачало <> Неопределено И Не ЗначениеЗаполнено(ПараметрНачало.Значение) Тогда
        ПараметрНачало.Значение = '00010101'; // Минимальная дата в 1С
        ПараметрНачало.Использование = Истина;
    КонецЕсли;
    
    Если ПараметрКонец <> Неопределено И Не ЗначениеЗаполнено(ПараметрКонец.Значение) Тогда
        ПараметрКонец.Значение = '29991231 23:59:59'; // Максимальная дата в 1С
