...
Функция ПолучитьОписаниеУзлаОбмена(СсылкаНаУзелОбмена) Экспорт
	...
	|	ИнтеграцияСМагазинамиСоцСетей.СпособАвторизации КАК СпособАвторизации
	|ИЗ
	...
	ИнтеграцияСМагазинамиСоцСетейКлиентСервер.АвторизацияВКонтактеПараметрыНовый( ВыборкаДетальныеЗаписи.СпособАвторизации );

	ИнтеграцияСМагазинамиСоцСетейСервер.АвторизацияВКонтактеПараметрыИзБезопасногоХранилищеПолучить( СсылкаНаУзелОбмена, АвторизацияВКонтактеПараметры );
	
	ОписаниеУзелОбмена = Новый Структура;
	
	ОписаниеУзелОбмена.Вставить("Ссылка" , ВыборкаДетальныеЗаписи.Ссылка);
	ОписаниеУзелОбмена.Вставить("КодПриложения" , ВыборкаДетальныеЗаписи.КодПриложения);
	
	ОписаниеУзелОбмена.Вставить("СпособАвторизации", ВыборкаДетальныеЗаписи.СпособАвторизации);
	
	ОписаниеУзелОбмена.Вставить("ТокенДоступа", Строка( АвторизацияВКонтактеПараметры.access_token ));
	ОписаниеУзелОбмена.Вставить("ОкончанияДействияТокенаДоступа", АвторизацияВКонтактеПараметры.ОкончанияДействияТокенаДоступа );

	ОписаниеУзелОбмена.Вставить("ТокенДоступаСообщества", Строка( АвторизацияВКонтактеПараметры.access_token_community ) );
	ОписаниеУзелОбмена.Вставить("ОкончанияДействияТокенДоступаСообщества", АвторизацияВКонтактеПараметры.ОкончанияДействияТокенДоступаСообщества );
...
Процедура ДобавитьЗначениеДополнительныхПолейОбъектовМагазина(УзелИнформационнойБазы, СсылкаНаОбъект, Значение, ИмяПоля, ДатаЗагрузки)
	...
	МенеджерЗаписи = РегистрыСведений.ЗначениеДополнительныхПолейОбъектовДляМагазинаСоцСетей.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.УзелИнформационнойБазы = УзелИнформационнойБазы;
	МенеджерЗаписи.Ссылка = СсылкаНаОбъект;
...
Функция ПолучитьСсылкуНаСтрокуАвторизации(НастройкиУзла, ЭтоСообщество) Экспорт
	...
	Если НастройкиУзла.МагазинДляИнтеграции = ПредопределенноеЗначение("Перечисление.МагазиныСоцСетейДляИнтеграции.VK") Тогда
		...
		Если ЭтоСообщество Тогда
			СтрШаблон = "https://oauth.vk.com/authorize?v=%1&client_id=%2&group_ids=%3&scope=messages,manage,docs&display=page&response_type=token";
			Возврат СтрШаблон(СтрШаблон, ПолучитьАктуальнуюВерсиюAPI(), НастройкиУзла.КодПриложения, НастройкиУзла.ИдентификаторГруппы);
		Иначе
			СтрШаблон = "https://oauth.vk.com/authorize?v=%1&client_id=%2&scope=groups,market,photos,offline,docs&display=page&response_type=token";
...
Процедура ВыполнитьОбменИнтерактивно(СсылкаНаУзелОбмена, ЭтоПолныйОбмен = Ложь, ЭтоФоновоеЗадание = Ложь) Экспорт
	...
	Попытка
		...
		МенеджерЗаписи = РегистрыСведений.ПоследняяУспешнаяОперацияОбменаДляМагазинаСоцСетей.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.УзелИнформационнойБазы = ОписаниеУзелОбмена.Ссылка;
		
		Если ОписаниеУзелОбмена.ТребуетсяРучноеПодтверждение И ЭтоФоновоеЗадание Тогда
			МенеджерЗаписи.Прочитать();
		КонецЕсли;
		
		МенеджерЗаписи.ДатаОперации = ОписаниеУзелОбмена.ТекущаяДата;
		МенеджерЗаписи.Записать();
...
Процедура ОбработатьПодборкиТоваров(ОписаниеУзелОбмена, ЭтоПолныйОбмен = Ложь) Экспорт
	...
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		МенеджерЗаписи = РегистрыСведений.СопоставлениеИдентификаторовМагазиновСоцСетей.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.УзелИнформационнойБазы = ВыборкаДетальныеЗаписи.УзелИнформационнойБазы;
		МенеджерЗаписи.ТипИдентификатора = ВыборкаДетальныеЗаписи.ТипИдентификатора;
		МенеджерЗаписи.Ссылка = ВыборкаДетальныеЗаписи.Ссылка;
...
Процедура СоздатьРегламентноеЗаданиеОбмена(СсылкаНаУзелОбмена) Экспорт
	...
	Если ТипЗнч( СсылкаНаУзелОбмена ) = Тип( "ПланОбменаСсылка.ИнтеграцияСМагазинамиСоцСетей" ) Тогда
		УзелОбменаОбъект = СсылкаНаУзелОбмена.ПолучитьОбъект();
		УзелОбменаСсылка = СсылкаНаУзелОбмена;
	Иначе
		УзелОбменаОбъект = СсылкаНаУзелОбмена;
		УзелОбменаСсылка = УзелОбменаОбъект.Ссылка;
	КонецЕсли;

	Если УзелОбменаОбъект.ИспользоватьРегламентныеЗадания <> Истина Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ УзелОбменаОбъект.ВыгружатьИзмененияНоменклатуры
		И НЕ УзелОбменаОбъект.ВыгружатьЦены
		И НЕ УзелОбменаОбъект.ЗагружатьЗаказы
		И НЕ УзелОбменаОбъект.ВыгружатьОстаток Тогда
		Возврат;
	КонецЕсли;
	
	Задание = РегламентныеЗаданияСервер.Задание(УзелОбменаОбъект.ИдентификаторРегламентногоЗадания);
	Если Задание = Неопределено Тогда
		
		РасписаниеРегламентногоЗадания = ПолучитьРасписаниеРегламентногоЗадания(УзелОбменаСсылка);
		
		Параметры = Новый Массив;
		Параметры.Добавить(УзелОбменаСсылка);
		...
		ПараметрыЗадания.Вставить("Наименование", "ВКонтакте: " + УзелОбменаОбъект.Наименование);
...
Процедура УдалитьРегламентноеЗаданиеОбмена(СсылкаНаУзелОбмена, ПересоздатьЗадание = Ложь) Экспорт
	...
	Если ТипЗнч( СсылкаНаУзелОбмена ) = Тип( "ПланОбменаСсылка.ИнтеграцияСМагазинамиСоцСетей" ) Тогда
		ИдентификаторРегламентногоЗадания = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		СсылкаНаУзелОбмена.Ссылка, "ИдентификаторРегламентногоЗадания" );
	Иначе
		ИдентификаторРегламентногоЗадания = СсылкаНаУзелОбмена.ИдентификаторРегламентногоЗадания;	
	КонецЕсли;
	...
	Для Каждого КлючЗначение Из СоответствиеИдентификаторов Цикл 
		...
		Если ОписаниеНоменклатуры.Свойство("Актуальна") Тогда
			Если ОписаниеНоменклатуры.Свойство("Актуальна") И ОписаниеНоменклатуры.Свойство("deleted") Тогда
				...
				ИначеЕсли ОписаниеНоменклатуры.Актуальна = Ложь И ОписаниеНоменклатуры.deleted = "0"
				И ОписаниеНоменклатуры.Свойство("item_id")
				Тогда
...
Процедура ДобавитьОтложеннуюОперацию(ИмяОперации, СохраненнаяОперация, ПодтверждениеОперацииСтрокой, ОписаниеУзелОбмена)
	...
	МенеджерЗаписи.УзелИнформационнойБазы = ОписаниеУзелОбмена.Ссылка;
	МенеджерЗаписи.ДатаОперации = ОписаниеУзелОбмена.ТекущаяДата;
	МенеджерЗаписи.ТребуетсяРучноеПодтверждение = Истина;
	МенеджерЗаписи.ПодтверждениеОперацииСтрокой = ПодтверждениеОперацииСтрокой;
	МенеджерЗаписи.ИмяОперации = ИмяОперации;
	МенеджерЗаписи.СохраненнаяОперацияСтрокой = ЗначениеВСтрокуВнутр(СохраненнаяОперация);
	МенеджерЗаписи.Записать();
...
Процедура ПродолжитьОтложеннуюОперациюВФоне(Параметры, АдресХранилища = Неопределено) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		МенеджерЗаписи = РегистрыСведений.ПоследняяУспешнаяОперацияОбменаДляМагазинаСоцСетей.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.УзелИнформационнойБазы = Параметры.СсылкаНаУзелОбмена;
		МенеджерЗаписи.Удалить();
...
Процедура ОбработатьЦеныТоваров(СоответствиеИдентификаторов, РезультатЗапросаЦена, ВыгружатьЦены)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		...
		Попытка
			Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.СтараяЦена)
			И ВыборкаДетальныеЗаписи.СтараяЦена > ВыборкаДетальныеЗаписи.ОсновнаяЦена Тогда
...
Процедура УдалитьФайлыТовараПоИдентификаторам(МассивИдентификаторов, УзелИнформационнойБазы)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		МенеджерЗаписи = РегистрыСведений.СопоставлениеФайловДляМагазинаСоцСетей.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.УзелИнформационнойБазы = ВыборкаДетальныеЗаписи.УзелИнформационнойБазы;
		МенеджерЗаписи.Номенклатура = ВыборкаДетальныеЗаписи.Номенклатура;
		МенеджерЗаписи.Характеристика = ВыборкаДетальныеЗаписи.Характеристика;
		МенеджерЗаписи.Файл = ВыборкаДетальныеЗаписи.Файл;
		МенеджерЗаписи.Удалить();
	КонецЦикла;
...
Процедура ДобавитьХешКонтролируемыхРеквизитовМагазина(УзелИнформационнойБазы, Ссылка, Хеш)
	...
	МенеджерЗаписи = РегистрыСведений.ХешКонтролируемыхРеквизитовДляМагазинаСоцСетей.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.УзелИнформационнойБазы = УзелИнформационнойБазы;
...
Процедура ДобавитьОстаткиТоваровМагазина(УзелИнформационнойБазы, Номенклатура, Количество, Характеристика = Неопределено, СтруктурнаяЕдиница = Неопределено)
	...
	МенеджерЗаписи = РегистрыСведений.ОстаткиТоваровМагазинаСоцСетей.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.УзелИнформационнойБазы = УзелИнформационнойБазы;
...
// 
// Параметры:
//  ОписаниеУзелОбмена - Структура - Описание узел обмена:
//  ОписаниеФайла - Структура - 
//  ЭтоОсновнойФайл - Булево - Это основной файл
//  СтруктураЗапросаHTTP - Структура - Структура запроса HTTP:
// 
// Возвращаемое значение:
//  Неопределено, Строка - Обработать файл VK
...
//  ЭтоОсновнойФайл - Булево - Это основной файл
//  СтруктураЗапросаHTTPДляФайла - Структура - Структура запроса HTTPДля файла:
// 
// Возвращаемое значение:
//  Неопределено, Структура - Загрузить файл VK:
// * Идентификатор - Строка - 
// * ПубличнаяСсылка - Произвольный - 
//@skip-check method-optional-parameter-before-required
...
// 
// Возвращаемое значение:
//  Неопределено - Получить адрес сервера файлов
...
Функция СоздатьСообщениеMultipart(ДвоичныеДанные,ИмяСообщения = "file",ИмяФайла = "file1",ТипMITE = "")Экспорт
	
	ДвоичныеДанные = СоздатьСообщениеMultipart_ДвоичныеДанные(ИмяСообщения, ИмяФайла, ДвоичныеДанные ,ТипMITE);
...
Функция СоздатьСообщениеMultipart_ДвоичныеДанные(ИмяСообщения, ИмяФайла, ДвоичныеДанные , ТипMITE = "")
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если ТипЗнч( СтруктураДанные.Номенклатура ) = Тип( "СправочникСсылка.Номенклатура" ) Тогда
		НоменклатураОбъект = СтруктураДанные.Номенклатура.ПолучитьОбъект();
	Иначе
		НоменклатураОбъект = СтруктураДанные.Номенклатура;
	КонецЕсли;

	// Наборы
	Если НЕ СтруктураДанные.Свойство("ЕдиницаИзмерения")
	И ТипЗнч(СтруктураДанные.Номенклатура)=Тип("СправочникСсылка.Номенклатура") Тогда
		НоменклатураЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта( СтруктураДанные.Номенклатура, "ЕдиницаИзмерения" );
		СтруктураДанные.Вставить("ЕдиницаИзмерения", НоменклатураЕдиницаИзмерения);
	КонецЕсли;
	Если НЕ СтруктураДанные.Свойство("Коэффициент") Тогда
		Если ТипЗнч(СтруктураДанные.ЕдиницаИзмерения)=Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
			...
			ЕдиницаИзмеренияКоэффициент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта( СтруктураДанные.ЕдиницаИзмерения, "Коэффициент" );
	...
	СтруктураДанные.Вставить("ЭтоУслуга", НоменклатураОбъект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга"));
	СтруктураДанные.Вставить("ЭтоЗапас", (НоменклатураОбъект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Запас")
	ИЛИ НоменклатураОбъект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат")));
	...
	ИначеЕсли ЗначениеЗаполнено(НоменклатураОбъект.ВидСтавкиНДС) Тогда
		СтруктураДанные.Вставить("СтавкаНДС", 
		Справочники.СтавкиНДС.СтавкаНДС(НоменклатураОбъект.ВидСтавкиНДС, 
		?(ЗначениеЗаполнено(СтруктураДанные.ДатаОбработки), 
		СтруктураДанные.ДатаОбработки, ТекущаяДатаСеанса())));
	Иначе
		
		ОрганизацияВидСтавкиНДСПоУмолчанию
		= ОбщегоНазначения.ЗначениеРеквизитаОбъекта( СтруктураДанные.Организация, "ВидСтавкиНДСПоУмолчанию" );
		
		СтруктураДанные.Вставить("СтавкаНДС", 
		Справочники.СтавкиНДС.СтавкаНДС(ОрганизацияВидСтавкиНДСПоУмолчанию, 
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура) И НоменклатураОбъект.ИспользоватьХарактеристики
		Тогда
		...
		НоменклатураОбъект.ПроверятьЗаполнениеХарактеристики);
	КонецЕсли;
	...
	ИначеЕсли СтруктураДанные.Свойство("ВидЦен") Тогда
		...
		Если СтруктураДанные.Свойство("ВидРабот") Тогда
			...
			Если НоменклатураОбъект.ФиксированнаяСтоимость Тогда
	...
	КонецЕсли;
	СтруктураДанные.Вставить("ВесЕдиницыТовара", НоменклатураОбъект.Вес);
	СтруктураДанные.Вставить("ОбъемЕдиницыТовара", НоменклатураОбъект.Объем);
	
	СтруктураДанные.Вставить("Склад", НоменклатураОбъект.Склад);
	СтруктураДанные.Вставить("Ячейка", НоменклатураОбъект.Ячейка);
	...
	И НоменклатураОбъект.ИспользоватьПартии
		Тогда
		...
		СтруктураДанные.ПроверятьЗаполнениеПартий = НоменклатураОбъект.ПроверятьЗаполнениеПартий;
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура)
	И НоменклатураОбъект.ИспользоватьСерииНоменклатуры Тогда
...
Функция ЗагрузитьНоменклатуру(ПараметрыФоновогоЗадания, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	ПараметрыКонтекст = Новый Структура;
	ОбработатьНоменклатуру(МассивНоменклатурДляОбработки, ОписаниеУзелОбмена, , ПараметрыКонтекст);
...
Процедура СоздатьПодборкиТоваров(ОписаниеУзелОбмена)
	...
	Если ТаблицаСоставаВиртуальныхКаталогов.Количество() Тогда
		...
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			МенеджерЗаписи = РегистрыСведений.НоменклатураХарактеристикДляМагазинаСоцСетейВиртуальныеКаталоги.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.УзелИнформационнойБазы = ОписаниеУзелОбмена.Ссылка;
			МенеджерЗаписи.ВиртуальныйКаталог = ВыборкаДетальныеЗаписи.ВиртуальныйКаталог;
			МенеджерЗаписи.Номенклатура = ВыборкаДетальныеЗаписи.Номенклатура;
			МенеджерЗаписи.Характеристика = ВыборкаДетальныеЗаписи.Характеристика;
			МенеджерЗаписи.Записать();
		КонецЦикла;
...
Процедура ДобавитьДополнительныеПоляФайлам(Файл, Высота, Ширина) Экспорт
	...
	МенеджерЗаписи = РегистрыСведений.ДополнительныеПоляФайловДляМагазинаСоцСетей.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Файл = Файл;
	МенеджерЗаписи.Высота = Высота;
	МенеджерЗаписи.Ширина = Ширина;
	МенеджерЗаписи.Записать();
...
ПараметрыКонтекст = Неопределено)
	...
	Если ОшибкаСоответствие <> Неопределено Тогда
		Если ОповещатьПользователя Тогда
			ОбработатьОшибку(ОписаниеУзелОбмена, СтруктураЗапросаHTTP.ТелоОтвета);
			ОбщегоНазначения.СообщитьПользователю(СтруктураЗапросаHTTP.ТелоОтвета);
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ЗаполнитьОсновныеПараметры(АдресРесурса, ОписаниеУзелОбмена, ИспользоватьТокенСообщества = Ложь) Экспорт
	...
	Иначе
		...
		Если ОписаниеУзелОбмена.Свойство( "ОкончанияДействияТокенаДоступа" )
		И ОписаниеУзелОбмена.Свойство( "СпособАвторизации" )
		Тогда
			
			Если ОписаниеУзелОбмена.СпособАвторизации = ИнтеграцияСМагазинамиСоцСетейКлиентСервер.СпособАвторизацииVKID()
			И ОписаниеУзелОбмена.ОкончанияДействияТокенаДоступа < ТекущаяДатаСеанса()
			Тогда
				ИнтеграцияСМагазинамиСоцСетейСервер.АвторизацияVKIDТокенДоступаОбновить( ОписаниеУзелОбмена );
			КонецЕсли;
...
Функция ПолучитьСписокГруппПользователя(ОписаниеУзелОбмена, ОшибкаТекст = "") Экспорт  
	...
	ОшибкаСоответствие = РезультатСоответствие.Получить("error");
	Если ОшибкаСоответствие <> Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(ОшибкаСоответствие.Получить("error_msg"));
		ОшибкаТекст = ОшибкаСоответствие.Получить("error_msg");
		Возврат Неопределено;
	КонецЕсли;
